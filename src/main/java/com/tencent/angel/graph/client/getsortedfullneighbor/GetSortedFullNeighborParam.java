...
public class GetSortedFullNeighborParam extends GetParam {
	...
	public GetSortedFullNeighborParam(int matrixId, long[] nodeIds, int[] types) {
		super(matrixId);
	...
	public GetSortedFullNeighborParam() {
		this(-1, null, null);
	}
	...
	public List<PartitionGetParam> split() {
		...
		while (nodeIndex < nodeIds.length || partIndex < partitions.size()) {
			...
			if (length > 0) {
				partParams.add(new PartGetSortedFullNeighborParam(matrixId,
